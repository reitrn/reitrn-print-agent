...
printers. Full-featured variant â€” connects to Firebase AND serves a local HTTP(S)
...
- No native printer module in production â€” printing goes through Windows fallbacks
...
- Companion to **ReturnHub** (app.reitrn.com). Two print paths:
  ...
  2. **Local server** on `127.0.0.1:3010` (HTTPS using wildcard cert for
     `local.reitrn.com` in `assets/`, falls back to HTTP if certs missing) â€”
...
- `npm start` â€” run locally
- `npm run build` â€” Windows installer + portable (`electron-builder --win`)
- GitHub Actions workflow builds and releases the installer on push

## Key files

- `main.js` â€” tray/window, local server, label builders (`buildZPL`/`buildTSPL`),
  `renderJob()`, IPC
- `firebase-listener.js` â€” Firestore listener, station registration, job claiming
- `printer.js` â€” printer listing + raw printing fallback chain

## Decisions & gotchas

- **Printer language auto-detected from printer name** (`detectLang`): TSC/TTP/TEx/TPx
  â†’ TSPL, everything else â†’ ZPL. ReturnHub sends both `zpl` + `tspl` fields; mobile
...
- **Raw print fallback chain**: direct Node write to `\\.\USBxxx` (port looked up
  via wmic, cached) â†’ PowerShell CreateFile P/Invoke â†’ `copy /b` â†’ WinSpool P/Invoke.
- **wmic only here** â€” this repo's `getInstalledPrinters` has NO `Get-Printer`
  PowerShell fallback; on Windows 11 22H2+ (wmic removed) printer listing fails.
  The fix exists in reitrn-lite-print-agent's `printer.js` â€” port it if needed.
...
- `/print` responds `202` immediately and prints in the background â€” don't make
